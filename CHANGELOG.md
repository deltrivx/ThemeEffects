...
- **移除页面左上角多余的 `\n` 字符**：`ThemeEffects_Loader.page` 的光标关键样式注入使用 PHP 单引号字符串，`\n` 不会被解析为换行，而是作为字面文本原样输出。其中 `</style>` 与 `</script>` 两处的 `\n` 落在标签之外，被浏览器当作正文渲染，在所有页面左上角顶部显示两个可见的 `\n`
- **修复输入框光标规则失效**：同一处注入中，`\n` 进入 `<style>` 内部后被 CSS 解析为转义字符，使紧随其后的选择器变成 `nhtml.ucwc-cursor-custom input...`，导致输入框、文本域与可编辑区域的 `cursor: text` 规则从未生效
- 五处 `echo '...\n'` 统一改为 `echo '...' . "\n"`，字符串主体保持单引号以避免变量插值，换行改由双引号提供

### 文件改动

| 文件 | 改动 |
|------|------|
| `ThemeEffects_Loader.page` | 5 处单引号字面 `\n` 改为拼接双引号换行 |
| `versions/index.json` | 新增 v2.7.4，v2.7.3 降级为 stable |
| `theme.effects.plg` | 版本号更新至 v2.7.4 |
| `README.md` | 当前正式版与安装命令更新至 v2.7.4 |

## v2.7.3 - 2026-08-01

### 仪表盘运行状态筛选修复
- 修复 Docker 容器切换到“仅显示已启动”后，已停止容器仍被主题样式强制显示的问题
- 同步修复虚拟机卡片的相同 CSS 覆盖风险，恢复 Unraid 原生 `display:none` 筛选行为
- Docker/VM 卡片继续使用 1.8.x 浮动排版和 137px 内容宽度，不改变当前仪表盘布局
- 发布校验新增回归检查，禁止仪表盘卡片规则再次使用 `display: block !important`
