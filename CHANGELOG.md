...
**正式版 v2.8.9**：恢复 Community Applications 搜索栏对 Unraid 盒子模式的原生宽度约束。

- 修复主题样式用 `max-width: 100%` 覆盖 CA 原生 `1920px` 盒子宽度的问题。
- 搜索栏外层重新与 `#displaybox` 保持相同的最大宽度和水平居中方式。
- 撤销针对内层 `.searchArea` 的实验性收拢规则，保留按钮原生排列与间距。

## v2.8.8 - 2026-08-08

### 全局版本信息

**正式版 v2.8.8**：优化全局版本信息弹窗，并撤销 Apps 搜索栏的实验性收拢修改。

- 保留全局版本信息弹窗的更新进度与重装入口。
- 移除 Apps 搜索栏相关实验性布局覆盖，恢复稳定版本行为。

## v2.8.7 - 2026-08-08

### 全局更新进度

**正式版 v2.8.7**：让全局左上角版本小窗与配置页使用同一套异步更新反馈。

- 更新弹窗新增 OTA / 全量任务的百分比进度、当前阶段和滚动日志。
- 复用 `ucwc-update.php` 的后台任务与 `job_status` 轮询，避免只显示“任务已启动”。
- 增加进度查询短暂失败的自动重试，以及完成、失败和刷新页面操作。

## v2.8.6 - 2026-08-08

### 概述
**正式版 v2.8.6**：优化左上角 ThemeEffects 版本小窗的版本同步与更新操作入口。

### 变更
- 版本按钮同步显示当前已安装版本，修复安装后仍显示历史版本的问题。
- 最新版状态下，点击版本小窗首行直接查看当前版本更新日志。
- 检测到新版本时，首行显示远程版本并打开 OTA / 全量更新操作。
- 移除重复的“查看当前更新日志”菜单项。
- 不包含视觉样式、尺寸、配色或布局调整。
...
### 概述
**正式版 v2.8.5**：统一 ThemeEffects 更新弹窗与系统弹窗的尺寸和布局。

### 变更
- 放大当前版本更新日志弹窗，使用接近系统弹窗的视口级宽高并保持移动端适配。
- 放大版本管理和后续更新弹窗，取消过小的日志与版本列表高度限制。
- 将弹窗正文改为弹性布局和可滚动内容区，避免更新内容被挤压或裁剪。
- 修复版本管理弹窗打开时使用 `display: block` 覆盖 flex 布局的问题。
...
### 概述
**正式版 v2.8.3**：恢复 Dashboard 各卡片原生标题与内容显示方式。

### 变更
- 撤销会统一不同卡片标题高度、网格、宽度和换行方式的强制 CSS。
- 撤销 Dashboard 内容区域统一垂直居中规则，避免文字重合和卡片间相互影响。
- 移除 CPU 卡片专用的移动端强制排版，恢复各卡片自己的响应式布局。
- 同步更新 v2.8.1/v2.8.2 快照校验，并补充 v2.8.3 发布快照。

## v2.8.2 - 2026-08-05

### 概述
**正式版 v2.8.2**：新增独立滚动条颜色设置并修复颜色保存。

### 变更
- 新增滚动条全局颜色和页面覆盖设置
- 修复颜色块选择后点击应用颜色被还原
- 分离字体与滚动条配置保存，避免分段互相覆盖

## v2.8.1 - 2026-08-04

### 概述
**正式版 v2.8.1**：修复 Dashboard 卡片布局与移动端标题显示。

### 变更
- 修复 Dashboard 卡片标题对齐、遮挡和内容区纵向居中
- 恢复 Docker/VM 原生排列及停止项筛选
- 修复移动端处理器型号双行显示
