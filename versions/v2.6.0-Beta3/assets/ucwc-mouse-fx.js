...
(function () {
  ...
  var CURSOR_OPTS = ["system", "dot", "cross", "neon", "none"];
  var api = {
  ...
  /** Apply Windows-like system pointer style via html data attr + classes. */
  function applyCursorStyle() {
    try {
      ...
      api.cursor = cur;
      // Always clear previous cursor classes
      ...
      // Cursor style is independent of canvas FX enable — user can restyle pointer alone.
      // When MOUSE_FX is off we still honor MOUSE_CURSOR if not system.
      root.setAttribute("data-ucwc-cursor", cur);
      root.classList.add("ucwc-cursor-" + cur);
      if (cur === "system") {
        root.classList.remove("ucwc-cursor-custom");
      } else {
