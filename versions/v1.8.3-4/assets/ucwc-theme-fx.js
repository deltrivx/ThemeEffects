...
(function () {
  ...
  function api(action, extra, timeoutMs) {
    ...
    var ms = timeoutMs != null ? timeoutMs : isRead ? 45000 : 20000;
  ...
  function ensureProgressUi() {
    ...
    wrap.innerHTML =
      ...
      '<div class="ucwc-log" id="ucwc-out"></div>';
  ...
  function finishInstall(j) {
    setBusy(false);
    setProgress(100, j && j.ok === false ? "失败" : "完成", (j && (j.message || j.stage)) || "完成");
    updateBar(j);
    var tip = ((j && j.message) || "完成") + "\n请强制刷新 WebGUI（Ctrl+F5）。";
    if (j && j.page_may_vanish) {
      tip += "\n主题特效页可能已移除，将尝试返回仪表盘。";
      alert(tip);
      try {
  ...
  function runInstall(action, version) {
    ...
    setProgress(3, "启动任务", "正在提交安装请求…");
    var extra = { async: "1" };
    if (version) extra.version = version;
    // 12s: server only enqueues job (no GitHub in web request)
    api(action, extra, 12000)
      .then(function (j) {
        if (!j || !j.ok) {
          setBusy(false);
        ...
        if (j.async && j.job_id) {
          ...
          if (out) out.textContent = "job_id=" + j.job_id + "\n";
