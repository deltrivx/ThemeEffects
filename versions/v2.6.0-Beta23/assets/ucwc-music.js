...
(function (global) {
  ...
  function showResumeChip(label) {
    ...
    if (!chip) {
      ...
      bindChipDrag();
      // Clicks on chip never navigate Unraid pages
  ...
  function buildUi() {
    ...
    // Card shell: never let clicks bubble into Unraid navigation
    root.addEventListener(
      "click",
      function (ev) {
        var t = ev.target;
        if (!t) return;
        if (t.closest && t.closest("button, a, input, label, .ucwc-music-btn, .ucwc-music-list-item, .ucwc-music-seek")) {
          try {
            ev.stopPropagation();
          } catch (e1) {}
        }
