 ...
 * Cross-page: chip-styled mini window hosts Audio (same chip look); page chip/card remote it.
 * Sitewide: chip always visible; card ↔ chip ↔ host bi-directional sync.
 * Host closes when Unraid tab heartbeats stop.
 */
(function (global) {
  "use strict";
  ...
  var DASH_POS_KEY = "ucwc_music_dash_pos_v2";
  ...
  var resumeLockId = "";
  var listFetchSeq = 0;
  // Chip-window host (popup looks like chip; holds Audio across Unraid navigations)
  var HOST_CH = "ucwc-music-host-v1";
  var HOST_URL = "/plugins/theme.effects/assets/ucwc-music-host.html";
  var HOST_NAME = "ucwc_themeeffects_music_chip";
  var HOST_HB_KEY = "ucwc_music_host_hb_v1";
  var hostWin = null;
  var hostBc = null;
  var hostAlive = false;
  var hostState = null;
  ...
  function isUiPlaying() {
    // Host chip-window is source of truth when alive; else real page audio.
    if (hostAlive && hostState && hostState.playing) return true;
    if (hostAlive && hostState && hostState.playing === false) return false;
  ...
  function usingHost() {
    try {
      ...
      return !!(preferHost && isSitewidePlay() && hostAlive);
    }
  }
  ...
  function beatHostHeart() {
    try {
      localStorage.setItem(HOST_HB_KEY, String(Date.now()));
    } catch (e0) {}
    try {
      postToHost({ type: "heartbeat" });
    } catch (e1) {}
  }

  function startHostHeartbeat() {
    beatHostHeart();
    if (hostHbTimer) return;
    hostHbTimer = setInterval(function () {
      if (!isSitewidePlay()) return;
      beatHostHeart();
      // re-attach named chip window if it still exists after navigation
      try {
        if (preferHost && (!hostWin || hostWin.closed)) {
          var w = window.open("", HOST_NAME);
          if (w) {
            var href = "";
            try { href = String(w.location.href || ""); } catch (eH) { href = ""; }
  ...
  function onHostMessage(data) {
    ...
    if (data.type === "host-gone") {
      ...
      updatePlayBtn();
      syncSitewideChip(resumeIntent ? "小窗已关，点播放可再开" : "");
      return;
    }
    if (data.type === "host-ready" || data.type === "state") {
      hostAlive = true;
      hostState = data;
      if (typeof data.index === "number" && data.index >= 0 && data.index < (state.tracks.length || 1e9)) {
        state.index = data.index;
      } else if (data.id && state.tracks.length) {
        for (var i = 0; i < state.tracks.length; i++) {
          if (state.tracks[i] && state.tracks[i].id === data.id) {
            state.index = i;
            break;
          }
        }
      }
      if (typeof data.t === "number" && data.t > 0.5) pendingSeekTo = data.t;
      ...
      if (data.playing) {
        ...
        clearPlayRetries();
        // Page must not double-play
        try {
          if (audio && !audio.paused) audio.pause();
        } catch (e0) {}
        // Mirror progress on card without owning audio
        if (els.cur && typeof data.t === "number") els.cur.textContent = fmt(data.t);
      ...
      updateChipUi();
      syncSitewideChip();
      // Persist host truth into session so remount pins correct track
      try {
        writePlaySession({
          playing: !!data.playing,
          intent: !!(data.playing || resumeIntent),
          index: state.index,
          t: typeof data.t === "number" ? data.t : 0,
          id: data.id || (current() && current().id) || "",
          vol: state.volume,
  ...
  /** Open/reuse chip-styled mini window as audio host (user gesture preferred). */
  function ensureMusicHost(fromGesture) {
    ...
    try {
      ...
      if (hostWin) {
        var href = "";
        try {
          href = String(hostWin.location.href || "");
        } catch (eH) {
          href = "";
        }
        if (href.indexOf("ucwc-music-host.html") !== -1) {
        ...
        if (fromGesture) {
          try {
            hostWin.location = HOST_URL + "?v=22";
          } catch (eN) {}
          hostAlive = true;
          setTimeout(function () {
            ...
            var curT =
              pendingSeekTo > 0.5
                ? pendingSeekTo
                : audio && isFinite(audio.currentTime)
                  ? audio.currentTime
                  : 0;
            postToHost({
              type: "playAt",
    ...
    try {
      // Compact chip-sized window — same look as page chip
      hostWin = window.open(
        HOST_URL + "?v=22",
        HOST_NAME,
        "width=360,height=88,resizable=yes,scrollbars=no,status=no,location=no,menubar=no,toolbar=no"
      );
      if (hostWin) {
        hostAlive = true;
        setTimeout(function () {
          ...
          if ((!curT2 || curT2 < 0.5) && sess && typeof sess.t === "number") curT2 = sess.t;
          postToHost({
            type: "playAt",
            index: state.index,
            id: (t2 && t2.id) || (sess && sess.id) || "",
            autoPlay: true,
            t: curT2 || 0,
            volume: state.volume,
          });
  ...
  function hostPlayAt(idx, autoPlay, startAt) {
    if (!usingHost() && !ensureMusicHost(!!autoPlay)) return false;
    if (!hostAlive && !usingHost()) return false;
    var t = state.tracks[idx];
    state.index = idx;
    if (typeof startAt === "number" && startAt > 0.5) pendingSeekTo = startAt;
    postToHost({
      type: "playAt",
      index: idx,
      id: t && t.id ? t.id : "",
      autoPlay: autoPlay !== false,
      t: typeof startAt === "number" ? startAt : pendingSeekTo || 0,
      volume: state.volume,
    });
    if (autoPlay !== false) {
      resumeIntent = true;
  ...
  function hostCmd(type, extra) {
    if (!usingHost() && !hostAlive) {
      if (!ensureMusicHost(false)) return false;
    }
  ...
  function savePlaySession(forcePlaying) {
    try {
      var t = current();
      var livePlaying = !!(audio && !audio.paused) || !!(hostAlive && hostState && hostState.playing);
      if (livePlaying) resumeIntent = true;
      var playing = typeof forcePlaying === "boolean" ? forcePlaying : livePlaying;
      var id = t && t.id ? t.id : "";
      var curT = 0;
      if (hostAlive && hostState) {
        if (hostState.id) id = hostState.id;
        if (typeof hostState.t === "number" && hostState.t > 0.25) curT = hostState.t;
        if (typeof hostState.index === "number") state.index = hostState.index;
      }
      if (pendingSeekTo > 0.5 && id) curT = Math.max(curT, pendingSeekTo);
      if (audio && isFinite(audio.currentTime) && audio.currentTime > 0.25 && !(hostAlive && hostState && hostState.playing)) {
        // live page audio wins unless this is a failed seek-to-0 flash while we still have a target
      ...
      // Never clobber a good saved progress with 0 for the same track (src reload race)
      ...
      var payload = {
        ...
        vol: state.volume,
        sitewide: isSitewidePlay(),
        host: !!(hostAlive || (hostState && hostState.playing)),
        ts: Date.now(),
      };
  ...
  function savePlaySessionForNav() {
    try {
      ...
      lastNavSave = now;
      var live = !!(audio && !audio.paused) || !!(hostAlive && hostState && hostState.playing);
  ...
  function schedulePlayRetries(reason) {
    ...
    function tick() {
      ...
      playRetryTimer = setTimeout(function () {
        ...
        try {
          if (pendingSeekTo > 0.5 && audio && isFinite(audio.duration) && audio.duration > 0) {
            try {
  ...
  function bindMediaSession() {
    try {
      ...
      navigator.mediaSession.setActionHandler("pause", function () {
        if (usingHost() || hostAlive) hostCmd("pause");
  ...
  function syncSitewideChip(hint) {
    if (!enabled() || state.collapsed || !isSitewidePlay()) {
      hideResumeChip();
      return;
    }
    showResumeChip(hint || "");
  ...
  function updateChipUi() {
    ...
    if (chipEls.lrc) {
      ...
      else if (isUiPlaying()) chipEls.lrc.textContent = hostAlive ? "♪ 小窗播放中" : "♪ 播放中";
      else if (hostAlive) chipEls.lrc.textContent = "小窗已开 · 已暂停";
      else if (pendingResume && playRetryCount > 0) chipEls.lrc.textContent = "尝试续播…";
      else if (resumeIntent || gestureResumeWanted) chipEls.lrc.textContent = "已暂停 · 点击播放开小窗";
      else chipEls.lrc.textContent = "点击播放 · 开 chip 小窗";
    }
    if (chipEls.play) {
  ...
  function showResumeChip(label) {
    // Sitewide: chip always visible (dashboard + other pages). Card and chip stay in sync.
    if (!enabled() || !isSitewidePlay() || state.collapsed) {
    ...
    if (!chip) {
      ...
      if (chipEls.play) {
        ...
        chipEls.play.addEventListener("click", function (ev) {
          ...
          gestureResumeWanted = true;
          resumeIntent = true;
          if (isSitewidePlay()) ensureMusicHost(true);
          togglePlay();
          updateChipUi();
        });
      }
  ...
  function trackUrl(id) {
    ...
    return apiBase + "?action=stream&id=" + encodeURIComponent(id) + "&_v=" + v;
  }
  ...
  function updateMeta() {
    ...
    if (els.art && t.id && (state.cover.id !== t.id || (!state.cover.url && !state.cover.loading))) {
      loadCoverForCurrent();
    }
  }

  function updatePlayBtn() {
    var playing = isUiPlaying(); // real audio only
    if (els.play) {
      ...
      els.play.classList.toggle("is-paused", !playing);
    }
    updateChipUi();
  }
  ...
  function revokeCoverBlob() {
    try {
      if (state.cover && state.cover.blobUrl) {
        URL.revokeObjectURL(state.cover.blobUrl);
        state.cover.blobUrl = "";
      }
    } catch (e0) {}
  }
  ...
  function setCoverUrl(url, trackId) {
    ...
    img.onerror = function () {
      ...
      if (state.cover.id !== trackId) return;
      // keep art shell; only drop broken img
      try {
        if (img.parentNode) img.parentNode.removeChild(img);
      } catch (e2) {}
      els.art.classList.remove("has-cover");
    ...
    if (img.getAttribute("src") === finalUrl && img.complete && img.naturalWidth > 0) {
      markOk();
      return;
    }
    img.src = finalUrl;
  ...
  function loadCoverForCurrent() {
    ...
    // Do not wipe previous art until new one is ready (reduces flash)
    var reqUrl = apiBase + "?action=cover&id=" + encodeURIComponent(trackId) + "&fetch=1&_ts=" + Date.now();
    fetch(reqUrl, {
          ...
          .then(function (blob) {
            ...
            if (j.source && (String(j.source).indexOf("downloaded") === 0 || String(j.source).indexOf("remote") === 0 || String(j.source) === "embedded")) {
              ...
              if (tip) {
                setStatus(tip);
                setTimeout(function () {
                  if (state.error === tip) setStatus(state.tracks.length ? "共 " + state.tracks.length + " 首" : "");
                }, 2200);
              }
            }
            updateMediaSessionMeta();
  ...
  function tryPlayUnlocked(a, onBlocked) {
    ...
    function ok() {
      ...
      // Re-assert seek if autoplay started at 0 despite pendingSeekTo
      try {
        if (pendingSeekTo > 1 && a && isFinite(a.duration) && a.duration > 0 && (a.currentTime || 0) < 0.75) {
  ...
  function playAt(idx, autoPlay, startAt) {
    ...
    var t = current();
    if (!t) return;
    // Sitewide + chip-window host: never thrash local audio (stops track flash on navigate)
    if (isSitewidePlay() && preferHost && (usingHost() || hostAlive)) {
      if (hostPlayAt(state.index, autoPlay !== false, startAt)) {
    ...
    if (isSitewidePlay() && preferHost && autoPlay && hostAlive === false) {
      // Soft: if named window may exist, ping only — open requires gesture
    ...
    function applySeekOnly() {
      ...
      try {
        if (isFinite(a.duration) && a.duration > 0) {
          ...
          if (Math.abs((a.currentTime || 0) - capped) > 0.75) a.currentTime = capped;
      ...
      if (autoPlay) {
        ...
        if (!(a && !a.paused && lastSrcId === t.id && Math.abs((a.currentTime || 0) - (targetPos || 0)) < 1.5 && targetPos > 0)) {
          tryPlayUnlocked(a, function () {
            ...
            if (isDashboard()) setStatus("正在尝试自动续播…");
            else {
              setStatus("");
  ...
  function togglePlay() {
    ...
    if (isSitewidePlay() && preferHost) {
      ...
      if (usingHost() || hostAlive) {
        ...
        var tH =
          sessH && typeof sessH.t === "number"
            ? sessH.t
            : hostState && typeof hostState.t === "number"
              ? hostState.t
              : audio && isFinite(audio.currentTime)
                ? audio.currentTime
                : pendingSeekTo || 0;
  ...
  /** Prefer a real dashboard column so the card floats with other tiles, not a full-width banner. */
  function findDashHost() {
    ...
    // 2) Unraid multi-column boxes (never prefer bare #db-box outer first)
    var colSelectors = [
      "#db-box1",
      "#db_box1",
      "#db-box2",
    ...
    var i, el;
    for (i = 0; i < colSelectors.length; i++) {
      try {
        el = document.querySelector(colSelectors[i]);
        if (el) return el;
      } catch (e3) {}
    }
    // 3) Parent of first dashboard table (column-ish)
    try {
  ...
  function insertDashWrap(host, wrap) {
    ...
    if (!placed && saved && typeof saved.index === "number" && saved.index >= 0 && kids.length) {
      // Avoid always choosing 0 when we have no anchors and many siblings
      var at = Math.min(Math.max(0, saved.index), kids.length);
  ...
  function placeInDashboard(card) {
    ...
    if (wrap && wrap.parentNode && wrap.parentNode !== host) {
      ...
      if (!shouldMove) {
        // Keep stable parent; just ensure card is inside wrap
    ...
    // If wrap already in a connected host, leave DOM position alone
    wrap.classList.remove("ucwc-music-sitewide");
    if (card.parentNode !== wrap) {
      wrap.appendChild(card);
    }
    watchDashReorder(wrap);
    // Delay first save so early mount doesn't lock index 0 before layout settles
    setTimeout(saveDashPos, 600);
    setTimeout(saveDashPos, 1800);
    setTimeout(saveDashPos, 4500);
    return true;
  }
  ...
  function buildUi() {
    ...
    els.shuffle.addEventListener("click", function () {
      ...
      updateModeBtns();
      saveLs();
      if (usingHost() || hostAlive) hostCmd("config", { volume: state.volume, shuffle: state.shuffle, repeat: state.repeat });
    });
    ...
    function commitSeek() {
      ...
      if (usingHost() || hostAlive) {
        var durH = hostState && typeof hostState.t === "number" ? 0 : 0;
        // best-effort: use page audio duration if known, else assume seek ratio via host current if we stored dur — send absolute if audio has dur
        var absT = 0;
        if (audio && isFinite(audio.duration) && audio.duration > 0) absT = r * audio.duration;
        else if (hostState && typeof hostState.t === "number" && els.dur) {
          // fallback keep relative unknown — still send r* rough
          absT = r * Math.max(hostState.t, 1);
        }
        pendingSeekTo = absT;
        hostCmd("seek", { t: absT });
        syncLyrics(true);
  ...
  function maybeResumeOrAutoplay() {
    ...
    if (isSitewidePlay()) {
      ...
      showResumeChip();
      if (usingHost() || hostAlive) {
        // Host owns playback — pin UI from session/host, do NOT local playAt (no flash)
        resumeAttempted = true;
        markPendingResume(false);
        var hs = hostState || loadPlaySession();
        if (hs) {
          if (typeof hs.index === "number") state.index = resolveSessionIndex(hs);
          else if (hs.id) state.index = resolveSessionIndex(hs);
          if (typeof hs.t === "number" && hs.t > 0.5) pendingSeekTo = hs.t;
        }
        updateMeta();
        renderList();
        updatePlayBtn();
    ...
    if (resumeAttempted) {
      if (isSitewidePlay()) {
        syncSitewideChip(pendingResume || resumeIntent ? "点播放打开 chip 小窗" : "");
        updateChipUi();
        showResumeChip();
      }
      return;
    ...
    if (sessionWantsResume(sess) || resumeIntent) {
      if (isDashboard() || isSitewidePlay()) {
        resumeAttempted = true; // set FIRST to block concurrent mount/fetch re-entry
        resumeIntent = true;
        ...
        updateMeta();
        renderList();
        // Sitewide without host yet: pin UI only; wait for chip click to open host (no local thrash)
        if (isSitewidePlay() && preferHost && !hostAlive) {
          gestureResumeWanted = true;
          bindGestureUnlock();
          showResumeChip("点播放打开 chip 小窗续播");
          syncSitewideChip("点播放打开 chip 小窗续播");
          return;
        }
        playAt(idx, true, t);
        if (isSitewidePlay()) {
        ...
        setTimeout(function () {
          if (hostAlive || usingHost()) return;
          if (audio && audio.paused && (resumeIntent || sessionWantsResume(loadPlaySession()))) {
            if (pendingSeekTo > 0.5) {
              try {
                if (isFinite(audio.duration) && audio.duration > 0 && Math.abs(audio.currentTime - pendingSeekTo) > 1.25) {
                  audio.currentTime = Math.min(pendingSeekTo, audio.duration - 0.25);
                }
              } catch (e0) {}
            }
            markPendingResume(true);
  ...
  function mount() {
    ...
    if (!shouldShowCard()) {
      ...
      if (els.vol) els.vol.value = String(Math.round(state.volume * 100));
      updateModeBtns();
      // Sync button to live audio / host after cross-page mount
      if (audio && audio.paused && !(hostAlive && hostState && hostState.playing)) state.playing = false;
      updatePlayBtn();
      updateMeta();
      if (state.tracks.length) {
        loadLyricsForCurrent();
