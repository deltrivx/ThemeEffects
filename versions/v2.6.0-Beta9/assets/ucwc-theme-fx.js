...
(function () {
  ...
  function bindMouseFxControls(root) {
    ...
    // 自定义样式文件 / 热点：仅在 MOUSE_CURSOR=upload 时显示整行
    (function bindCursorUploadVisibility() {
      ...
      function findRowPair(node) {
        var pairs = [];
        if (!node) return pairs;
        var p = node;
        for (var hops = 0; p && hops < 10; hops++) {
          var tag = (p.tagName || "").toLowerCase();
          if (tag === "dd" || tag === "td") {
            pairs.push(p);
            var prev = p.previousElementSibling;
            if (prev) {
              var pt = (prev.tagName || "").toLowerCase();
              if (pt === "dt" || pt === "th") pairs.push(prev);
            }
            if (p.parentElement && (p.parentElement.tagName || "").toLowerCase() === "tr") {
              pairs.push(p.parentElement);
            }
            break;
          }
          if (tag === "tr" || tag === "dl") {
      ...
      function labelMatches(txt) {
        ...
        return (
          txt.indexOf("自定义样式文件") >= 0 ||
          txt.indexOf("自定义指针文件") >= 0 ||
          txt.indexOf("热点坐标") >= 0
        );
      }
      ...
      function syncCursorUploadRows() {
        ...
        var anchors = root.querySelectorAll(
          '#ucwc-mouse-cursor-upload, input[name="MOUSE_CURSOR_UPLOAD"], input[name="MOUSE_CURSOR_HOTSPOT_X"], input[name="MOUSE_CURSOR_HOTSPOT_Y"], [data-ucwc-cursor-upload-row], .ucwc-cursor-upload-only'
        );
        var seen = [];
        for (var i = 0; i < anchors.length; i++) {
          var n = anchors[i];
          if (n.classList && n.classList.contains("ucwc-cursor-upload-only")) {
            n.style.display = show ? "inline-flex" : "none";
          }
          if (n.getAttribute && n.getAttribute("data-ucwc-cursor-upload-row")) {
            n.style.display = show ? "inline-flex" : "none";
          }
          if (n.tagName && n.tagName.toLowerCase() === "input") {
            var nm = n.name || "";
            if (nm === "MOUSE_CURSOR_UPLOAD" || nm === "MOUSE_CURSOR_HOTSPOT_X" || nm === "MOUSE_CURSOR_HOTSPOT_Y") {
              try {
                n.disabled = !show;
                if (show) n.removeAttribute("disabled");
              } catch (eDis) {}
            }
          }
          var hosts = findRowPair(n);
          for (var h = 0; h < hosts.length; h++) {
            var host = hosts[h];
            if (!host || seen.indexOf(host) >= 0) continue;
            seen.push(host);
            host.style.display = show ? "" : "none";
          }
        }
        try {
          var dts = root.querySelectorAll("dt, th");
          for (var d = 0; d < dts.length; d++) {
            if (!labelMatches(dts[d].textContent)) continue;
            dts[d].style.display = show ? "" : "none";
            var dd = dts[d].nextElementSibling;
            if (dd) dd.style.display = show ? "" : "none";
            if (dts[d].closest) {
              var tr = dts[d].closest("tr");
