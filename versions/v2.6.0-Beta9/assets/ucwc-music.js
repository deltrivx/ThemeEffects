...
(function (global) {
  ...
  function applyChipPos() {
    ...
    var maxX = Math.max(8, (window.innerWidth || 800) - chip.offsetWidth - 8);
    var maxY = Math.max(8, (window.innerHeight || 600) - chip.offsetHeight - 8);
    var x = Math.min(maxX, Math.max(8, pos.x));
    var y = Math.min(maxY, Math.max(8, pos.y));
  ...
  function bindChipDrag() {
    ...
    var onMove = function (ev) {
      ...
      if (Math.abs(dx) + Math.abs(dy) > 4) chipDrag.moved = true;
      var x = chipDrag.ox + dx;
      var y = chipDrag.oy + dy;
      var maxX = Math.max(8, (window.innerWidth || 800) - chip.offsetWidth - 8);
      var maxY = Math.max(8, (window.innerHeight || 600) - chip.offsetHeight - 8);
    ...
    if (sessionWantsResume(sess) || resumeIntent) {
      if (isDashboard() || isSitewidePlay()) {
        ...
        if (!isDashboard() && isSitewidePlay()) {
          gestureResumeWanted = true;
          bindGestureUnlock();
          syncSitewideChip(audio && !audio.paused ? "" : "点击播放以续播");
        }
        return;
      }
    }
  ...
  function boot() {
    ...
    document.addEventListener("visibilitychange", function () {
      if (document.visibilityState === "visible") {
        // UI only; do not force another autoplay if already attempted
        mount();
      }
    });
  }
