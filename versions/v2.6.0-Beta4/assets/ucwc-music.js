...
(function (global) {
  ...
  /**
   * MUSIC_DASH_ONLY:
   *  true  → 仅仪表盘播放（卡片只在仪表盘挂载）
   *  false → 全站播放（任意页面显示卡片并可播）
   * 卡片位置始终为仪表盘流内嵌；全站时非仪表盘页也会挂一块同款卡片。
   */
  function shouldShow() {
    if (!enabled()) return false;
    var c = cfg();
    if (c.dash_only === false) return true; // 全站播放
    return isDashboard(); // 仅仪表盘播放
  }
  ...
  /** Find a host inside page content flow (dashboard preferred; site-wide fallback). */
  function findDashHost() {
    ...
    var table = document.querySelector("table.dashboard, table.share_status.dashboard, table.unraid");
  ...
  function placeInDashboard(card) {
    ...
    // Site-wide: keep card in flow near top content (not fixed overlay)
    if (!isDashboard()) {
      wrap.classList.add("ucwc-music-sitewide");
    } else {
  ...
  function buildUi() {
    ...
    // Mount into dashboard flow; fallback only if dash DOM not ready yet
    if (!placeInDashboard(root)) {
      // Keep off-screen until dash host appears (mount() retries)
      root.classList.add("ucwc-music-hidden");
  ...
  function mount() {
    if (!shouldShow()) {
      ...
      var orphan = document.getElementById("ucwc-music-dash-host");
      if (orphan && !orphan.querySelector("#ucwc-music-card")) {
        try {
    ...
    applyCfgDefaults();
    buildUi();
    // Ensure card lives in dashboard content, not as a page overlay
