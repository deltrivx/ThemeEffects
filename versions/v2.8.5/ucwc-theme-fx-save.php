...
function ucwc_fx_defaults() {
    return [
        ...
        "COLOR_TITLE" => "",
        "COLOR_LABEL" => "",
        "SCROLLBAR_TRACK_GLOBAL" => "#282a36",
        "SCROLLBAR_THUMB_GLOBAL" => "#5ab0ff",
        "SCROLLBAR_THUMB_HOVER_GLOBAL" => "#8ccfff",
...
function ucwc_fx_normalize_font(&$fx) {
    ...
    $fx["SCROLLBAR_TRACK_GLOBAL"] = ucwc_hex_ok($fx["SCROLLBAR_TRACK_GLOBAL"] ?? "") ?: "#282a36";
    $fx["SCROLLBAR_THUMB_GLOBAL"] = ucwc_hex_ok($fx["SCROLLBAR_THUMB_GLOBAL"] ?? "") ?: "#5ab0ff";
    $fx["SCROLLBAR_THUMB_HOVER_GLOBAL"] = ucwc_hex_ok($fx["SCROLLBAR_THUMB_HOVER_GLOBAL"] ?? "") ?: "#8ccfff";
    foreach (["DASHBOARD", "MAIN", "SHARES", "DOCKER", "VMS", "APPS", "USERS", "SETTINGS", "PLUGINS", "TOOLS"] as $page) {
        foreach (["TRACK", "THUMB", "THUMB_HOVER"] as $part) {
            $key = "SCROLLBAR_" . $part . "_" . $page;
            $fx[$key] = ucwc_hex_ok($fx[$key] ?? "");
        }
    }
}
...
function ucwc_fx_section($raw) {
    ...
    if (in_array($s, ["scrollbar", "scrollbars", "scroll", "滚动条"], true)) return "scrollbar";
...
function ucwc_fx_save($path, $fx) {
    $keys = ["PARTICLES","PARTICLES_COUNT","HUTAO","HUTAO_SIZE","HUTAO_POS","HUTAO_TYPE","HUTAO_BLUR","HUTAO_BLUR_LEVEL","REDUCE_MOTION","MOUSE_FX","MOUSE_STYLE","MOUSE_SIZE","MOUSE_INTENSITY","MOUSE_COLOR","MOUSE_CLICK_RIPPLE","MOUSE_CURSOR","MOUSE_CURSOR_HOTSPOT_X","MOUSE_CURSOR_HOTSPOT_Y","BG_MODE","BG_LOCAL_SLOT","BG_GALLERY","BG_CUSTOM_API","BG_BLUR","BG_BLUR_LEVEL","FONT_BODY","FONT_TITLE","FONT_TITLE_UPPER","FONT_SIZE","FONT_TITLE_SIZE","FONT_CUSTOM_BODY","FONT_CUSTOM_TITLE","FONT_LOCAL_BODY","FONT_LOCAL_TITLE","COLOR_PRESET","COLOR_TEXT","COLOR_TITLE","COLOR_LABEL","SCROLLBAR_TRACK_GLOBAL","SCROLLBAR_THUMB_GLOBAL","SCROLLBAR_THUMB_HOVER_GLOBAL","SCROLLBAR_TRACK_DASHBOARD","SCROLLBAR_THUMB_DASHBOARD","SCROLLBAR_THUMB_HOVER_DASHBOARD","SCROLLBAR_TRACK_MAIN","SCROLLBAR_THUMB_MAIN","SCROLLBAR_THUMB_HOVER_MAIN","SCROLLBAR_TRACK_SHARES","SCROLLBAR_THUMB_SHARES","SCROLLBAR_THUMB_HOVER_SHARES","SCROLLBAR_TRACK_DOCKER","SCROLLBAR_THUMB_DOCKER","SCROLLBAR_THUMB_HOVER_DOCKER","SCROLLBAR_TRACK_VMS","SCROLLBAR_THUMB_VMS","SCROLLBAR_THUMB_HOVER_VMS","SCROLLBAR_TRACK_APPS","SCROLLBAR_THUMB_APPS","SCROLLBAR_THUMB_HOVER_APPS","SCROLLBAR_TRACK_USERS","SCROLLBAR_THUMB_USERS","SCROLLBAR_THUMB_HOVER_USERS","SCROLLBAR_TRACK_SETTINGS","SCROLLBAR_THUMB_SETTINGS","SCROLLBAR_THUMB_HOVER_SETTINGS","SCROLLBAR_TRACK_PLUGINS","SCROLLBAR_THUMB_PLUGINS","SCROLLBAR_THUMB_HOVER_PLUGINS","SCROLLBAR_TRACK_TOOLS","SCROLLBAR_THUMB_TOOLS","SCROLLBAR_THUMB_HOVER_TOOLS","PERF_PROFILE","CLIENT_OPTIMIZED"];
    $lines = [];
    foreach ($keys as $k) {
...
/**
 * Copy custom cursor from Unraid local path (/mnt/user|user0) to assets/cursor-custom.{ext}.
 * Sets $outExt on success. Returns "" on success / empty path; error string on failure.
 */
function ucwc_handle_cursor_local_path($path, $assets_f, $assets_r, &$outExt) {
    global $log_path;
    $outExt = "";
    ...
    $okRoot = false;
    foreach (["/mnt/user", "/mnt/user0"] as $rootDir) {
        $rr = @realpath($rootDir);
    ...
    if ($size > 2 * 1024 * 1024) return "自定义指针不能超过 2MB。";
    if (!ucwc_is_cursor_file($real)) return "指针仅支持 png / webp / svg / cur / ico。";
    $name = strtolower(basename($real));
    $ext = pathinfo($name, PATHINFO_EXTENSION);
    $ext = preg_replace('/[^a-z0-9]/', '', $ext);
    $h = ucwc_peek_header($real, 16);
    if (substr($h, 0, 8) === "\x89PNG\r\n\x1a\n") $ext = "png";
    elseif (substr($h, 0, 4) === "RIFF") $ext = "webp";
    elseif (strpos(strtolower(ltrim($h)), "<svg") !== false || strpos(strtolower(ltrim($h)), "<?xml") === 0) $ext = "svg";
    elseif (strlen($h) >= 4 && ord($h[0]) === 0 && ord($h[1]) === 0 && ord($h[2]) === 2) $ext = "cur";
    ...
    @mkdir(dirname($dst_r), 0755, true);
    @mkdir(dirname($dst_f), 0755, true);
    if (!@copy($real, $dst_r)) return "复制自定义指针失败。";
    @chmod($dst_r, 0644);
    @copy($dst_r, $dst_f);
    @chmod($dst_f, 0644);
    if (!ucwc_has($dst_f) && !ucwc_has($dst_r)) return "保存后未找到 {$label}。";
    $outExt = $ext;
    if (!empty($log_path)) ucwc_log($log_path, "cursor-local-ok ext={$ext} size={$size} src={$real}");
    return "";
}
...
$doMascot = ($section === "all" || $section === "mascot");
    ...
    $doScrollbar = ($section === "all" || $section === "scrollbar");
...
if ($doScrollbar) {
    $hasScrollbarPost = isset($_POST["SCROLLBAR_TRACK_GLOBAL"]) || isset($_POST["SCROLLBAR_THUMB_GLOBAL"]) || isset($_POST["SCROLLBAR_THUMB_HOVER_GLOBAL"]);
    foreach (["DASHBOARD", "MAIN", "SHARES", "DOCKER", "VMS", "APPS", "USERS", "SETTINGS", "PLUGINS", "TOOLS"] as $scrollbarPage) {
        if (isset($_POST["SCROLLBAR_TRACK_" . $scrollbarPage]) || isset($_POST["SCROLLBAR_THUMB_" . $scrollbarPage]) || isset($_POST["SCROLLBAR_THUMB_HOVER_" . $scrollbarPage]) || isset($_POST["SCROLLBAR_AUTO_" . $scrollbarPage])) {
            $hasScrollbarPost = true;
            break;
        }
    }
    if ($hasScrollbarPost) {
        foreach (["TRACK_GLOBAL", "THUMB_GLOBAL", "THUMB_HOVER_GLOBAL"] as $globalPart) {
            $key = "SCROLLBAR_" . $globalPart;
            $fallback = ["TRACK_GLOBAL" => "#282a36", "THUMB_GLOBAL" => "#5ab0ff", "THUMB_HOVER_GLOBAL" => "#8ccfff"][$globalPart];
            $fx[$key] = ucwc_hex_ok($_POST[$key] ?? ($fx[$key] ?? $fallback)) ?: $fallback;
        }
        foreach (["DASHBOARD", "MAIN", "SHARES", "DOCKER", "VMS", "APPS", "USERS", "SETTINGS", "PLUGINS", "TOOLS"] as $page) {
            foreach (["TRACK", "THUMB", "THUMB_HOVER"] as $part) {
                $key = "SCROLLBAR_" . $part . "_" . $page;
                $autoKey = "SCROLLBAR_AUTO_" . $page;
                $fx[$key] = (($_POST[$autoKey] ?? "") === "yes") ? "" : ucwc_hex_ok($_POST[$key] ?? ($fx[$key] ?? ""));
            }
        }
        ucwc_fx_normalize_font($fx);
    }
...
if (!empty($doCursorUpload) && $err === "") {
    $cursorExt = "";
    $hadCursorUpload = isset($_FILES["MOUSE_CURSOR_UPLOAD"]) && is_array($_FILES["MOUSE_CURSOR_UPLOAD"])
    ...
    if ($err === "" && !$hadCursorUpload) {
        $localC = trim((string)($_POST["MOUSE_CURSOR_LOCAL_PATH"] ?? ""));
        if ($localC !== "") {
            $cerr = ucwc_handle_cursor_local_path($localC, $assets_f, $assets_r, $cursorExt);
            if ($cerr !== "") {
                $err = $cerr;
            } elseif ($cursorExt !== "") {
                $fx["MOUSE_CURSOR"] = "upload";
    ...
    // If user chose upload but no asset exists and no new file/path, keep selection; Loader falls back.
    if ($err === "" && ($fx["MOUSE_CURSOR"] ?? "") === "upload") {
        ...
        if (!$hasCur) {
            $hadNew = $hadCursorUpload || trim((string)($_POST["MOUSE_CURSOR_LOCAL_PATH"] ?? "")) !== "";
            if (!$hadNew) {
                $err = "请先从电脑上传或填写 Unraid 本地路径（自定义指针，png/webp/svg/cur/ico，≤2MB）。";
            }
        }
    }
}
...
$ok = ($saved && $err === "");
$sectionLabel = ["bg" => "背景", "particles" => "粒子", "mouse" => "鼠标特效", "mascot" => "吉祥物", "font" => "字体", "scrollbar" => "滚动条", "perf" => "性能", "all" => "主题特效"][$section] ?? "主题特效";
ucwc_json([
    "ok" => $ok,
    "saved" => (bool)$saved,
    "section" => $section,
