...
(function () {
  ...
  function checkThemeUpdate() {
    ...
    window.fetch("https://raw.githubusercontent.com/deltrivx/ThemeEffects/main/versions/index.json")
      .then(function (response) { return response.ok ? response.json() : Promise.reject(new Error("HTTP " + response.status)); })
      .then(function (index) {
        var remote = index.latest_version || index.default || "未知";
        status.textContent = remote === THEME_INFO.version ? "已是最新版本" : "远程版本：" + remote;
      })
      .catch(function () { status.textContent = "无法检查远程版本"; });
  }
  ...
  function showThemeInfo() {
    ...
    menu.innerHTML =
      ...
      '<div class="ucwc-theme-info-item"><span>当前版本</span><strong id="ucwc-theme-update-status">检查中…</strong></div>' +
      '<div class="ucwc-theme-info-separator" role="separator"></div>' +
      '<button class="ucwc-theme-info-link" type="button" role="menuitem" data-action="changelog"><span class="ucwc-theme-info-link-label">查看当前更新日志</span></button>' +
    ...
    menu.querySelector('[data-action="changelog"]').addEventListener("click", function () {
      closeThemeInfo();
      showThemeChangelog();
    });
