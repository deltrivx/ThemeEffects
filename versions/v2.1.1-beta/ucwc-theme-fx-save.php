...
function ucwc_fx_normalize_font(&$fx) {
    $bodies = ["system", "rajdhani", "noto_sc", "yahei", "inter", "rubik", "source_han"];
    $titles = ["orbitron", "match_body", "system", "noto_sc", "yahei", "inter", "rubik", "exo2", "source_han"];
...
function ucwc_fx_save($path, $fx) {
    $keys = ["PARTICLES","PARTICLES_COUNT","HUTAO","HUTAO_SIZE","HUTAO_POS","HUTAO_TYPE","HUTAO_BLUR","REDUCE_MOTION","BG_MODE","BG_LOCAL_SLOT","BG_GALLERY","BG_CUSTOM_API","BG_BLUR","FONT_BODY","FONT_TITLE","FONT_TITLE_UPPER","FONT_SIZE","FONT_TITLE_SIZE","COLOR_PRESET","COLOR_TEXT","COLOR_TITLE","COLOR_LABEL"];
    $lines = [];
    foreach ($keys as $k) {
...
$ok = ($saved && $err === "");
$sectionLabel = ["bg" => "背景", "particles" => "粒子", "mascot" => "吉祥物", "font" => "字体", "all" => "主题特效"][$section] ?? "主题特效";
ucwc_json([
    "ok" => $ok,
    "saved" => (bool)$saved,
    "section" => $section,
