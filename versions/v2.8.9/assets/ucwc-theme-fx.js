...
(function () {
  ...
  function sectionOfButton(btn) {
    ...
    if (!s && btn.id) {
      ...
      else if (btn.id.indexOf("-scrollbar") >= 0) s = "scrollbar";
    ...
    if (["bg", "particles", "mouse", "mascot", "font", "scrollbar", "perf", "service", "all"].indexOf(s) < 0) {
      s = "all";
    }
    return s;
  }
  ...
  function syncUi() {
    try {
      ...
      // Mouse custom cursor file + hotspot — same hideRow path as BG_UPLOAD / wallpaper custom
      var mouseCursor = root.querySelector('select[name="MOUSE_CURSOR"]') || root.querySelector("#ucwc-mouse-cursor");
      var mouseCursorUpload = root.querySelector('input[name="MOUSE_CURSOR_UPLOAD"]') || root.querySelector("#ucwc-mouse-cursor-upload");
      var mouseCursorLocal = root.querySelector('input[name="MOUSE_CURSOR_LOCAL_PATH"]') || root.querySelector("#ucwc-mouse-cursor-local-path");
      var mouseHotX = root.querySelector('input[name="MOUSE_CURSOR_HOTSPOT_X"]') || root.querySelector("#ucwc-mouse-hotspot-x");
      var mouseHotY = root.querySelector('input[name="MOUSE_CURSOR_HOTSPOT_Y"]') || root.querySelector("#ucwc-mouse-hotspot-y");
      var mouseCursorWrap = root.querySelector("#ucwc-mouse-cursor-upload-wrap");
      var mouseHotWrap = root.querySelector("#ucwc-mouse-hotspot-wrap");
      var mouseCursorIsUpload = !!(mouseCursor && String(mouseCursor.value || "") === "upload");
      hideRow(mouseCursorUpload, !mouseCursorIsUpload);
      hideRow(mouseCursorLocal, !mouseCursorIsUpload);
      hideRow(mouseHotX, !mouseCursorIsUpload);
      // wrappers (if hideRow only hit inner input, force wrap visibility cleanly)
      if (mouseCursorWrap) {
        if (mouseCursorIsUpload) {
          mouseCursorWrap.classList.remove("ucwc-row-hidden");
          mouseCursorWrap.style.removeProperty("display");
          mouseCursorWrap.style.removeProperty("visibility");
          mouseCursorWrap.style.removeProperty("height");
          mouseCursorWrap.style.removeProperty("max-height");
          mouseCursorWrap.style.removeProperty("overflow");
        }
      }
      if (mouseHotWrap) {
        if (mouseCursorIsUpload) {
          mouseHotWrap.classList.remove("ucwc-row-hidden");
          mouseHotWrap.style.removeProperty("display");
          mouseHotWrap.style.removeProperty("visibility");
          mouseHotWrap.style.display = "inline-flex";
        }
      }
      try {
        if (mouseCursorUpload) {
          mouseCursorUpload.disabled = !mouseCursorIsUpload;
          if (mouseCursorIsUpload) mouseCursorUpload.removeAttribute("disabled");
        }
        if (mouseCursorLocal) {
          mouseCursorLocal.disabled = !mouseCursorIsUpload;
          if (mouseCursorIsUpload) mouseCursorLocal.removeAttribute("disabled");
        }
        if (mouseHotX) {
          mouseHotX.disabled = !mouseCursorIsUpload;
          if (mouseCursorIsUpload) mouseHotX.removeAttribute("disabled");
        }
        if (mouseHotY) {
          mouseHotY.disabled = !mouseCursorIsUpload;
          if (mouseCursorIsUpload) mouseHotY.removeAttribute("disabled");
        }
      } catch (eCurEn) {}
  ...
  function wireColorPair(pickerId, inputName, form, onChange) {
    ...
    function syncPickerValue() {
      var value = normHex(picker.value, "");
      if (!value) return;
      input.value = value;
    ...
    picker.addEventListener("input", syncPickerValue);
    picker.addEventListener("change", syncPickerValue);
  ...
  function bindFormUi() {
    try {
      ...
      function hasUploadFile(section) {
        ...
        var cursor = form.querySelector('input[name="MOUSE_CURSOR_UPLOAD"]');
      ...
      form.addEventListener("submit", function (ev) {
        ...
        // Native color controls can update without dispatching a text-input event on Unraid.
        // Copy picker values immediately before serializing the active section.
        if (section === "scrollbar" || section === "all") {
          try { syncScrollbarFormValues(form); } catch (eScrollbarSubmitSync) {}
        }
        ...
        var fieldSets = {
          ...
          mouse: ["MOUSE_FX", "MOUSE_STYLE", "MOUSE_SIZE", "MOUSE_SIZE_COMMIT", "MOUSE_INTENSITY", "MOUSE_INTENSITY_COMMIT", "MOUSE_COLOR", "MOUSE_COLOR_MODE", "MOUSE_CLICK_RIPPLE", "MOUSE_CURSOR", "MOUSE_CURSOR_HOTSPOT_X", "MOUSE_CURSOR_HOTSPOT_Y", "MOUSE_CURSOR_UPLOAD"],
          ...
          scrollbar: [
            "SCROLLBAR_TRACK_GLOBAL",
            "SCROLLBAR_THUMB_GLOBAL",
            "SCROLLBAR_THUMB_HOVER_GLOBAL",
            "SCROLLBAR_TRACK_DASHBOARD",
            "SCROLLBAR_THUMB_DASHBOARD",
            "SCROLLBAR_THUMB_HOVER_DASHBOARD",
            "SCROLLBAR_TRACK_MAIN",
            "SCROLLBAR_THUMB_MAIN",
            "SCROLLBAR_THUMB_HOVER_MAIN",
            "SCROLLBAR_TRACK_SHARES",
            "SCROLLBAR_THUMB_SHARES",
            "SCROLLBAR_THUMB_HOVER_SHARES",
            "SCROLLBAR_TRACK_DOCKER",
            "SCROLLBAR_THUMB_DOCKER",
            "SCROLLBAR_THUMB_HOVER_DOCKER",
            "SCROLLBAR_TRACK_VMS",
            "SCROLLBAR_THUMB_VMS",
            "SCROLLBAR_THUMB_HOVER_VMS",
            "SCROLLBAR_TRACK_APPS",
            "SCROLLBAR_THUMB_APPS",
            "SCROLLBAR_THUMB_HOVER_APPS",
            "SCROLLBAR_TRACK_USERS",
            "SCROLLBAR_THUMB_USERS",
            "SCROLLBAR_THUMB_HOVER_USERS",
            "SCROLLBAR_TRACK_SETTINGS",
            "SCROLLBAR_THUMB_SETTINGS",
            "SCROLLBAR_THUMB_HOVER_SETTINGS",
            "SCROLLBAR_TRACK_PLUGINS",
            "SCROLLBAR_THUMB_PLUGINS",
            "SCROLLBAR_THUMB_HOVER_PLUGINS",
            "SCROLLBAR_TRACK_TOOLS",
            "SCROLLBAR_THUMB_TOOLS",
            "SCROLLBAR_THUMB_HOVER_TOOLS",
            "SCROLLBAR_AUTO_DASHBOARD",
            "SCROLLBAR_AUTO_MAIN",
            "SCROLLBAR_AUTO_SHARES",
            "SCROLLBAR_AUTO_DOCKER",
            "SCROLLBAR_AUTO_VMS",
            "SCROLLBAR_AUTO_APPS",
            "SCROLLBAR_AUTO_USERS",
            "SCROLLBAR_AUTO_SETTINGS",
            "SCROLLBAR_AUTO_PLUGINS",
            "SCROLLBAR_AUTO_TOOLS",
          ],
  ...
  /** Stable channel for check_update: the index's top-level latest_version is authoritative. */
  function pickStableLatest(data) {
    var versions = (data && data.versions) || [];
    var latestId = (data && data.latest_version) || "";
    var i, v;
    if (latestId && !isBetaVersionId(latestId)) {
      if (data.latest && data.latest.id === latestId) return data.latest;
      for (i = 0; i < versions.length; i++) {
        v = versions[i];
        if (v && v.id === latestId) return v;
      }
      return null;
    }
    if (data && data.latest && !isBetaVersionId(data.latest.id)) {
      return data.latest;
    }
    // Compatibility only for older indexes that do not publish latest_version.
    for (i = 0; i < versions.length; i++) {
      v = versions[i];
  ...
  function renderChangelog() {
    ...
    body.innerHTML =
      '<div class="ucwc-ver-grid">' +
      '<div class="ucwc-ver-list">' +
      list +
      "</div>" +
  ...
  function ensureProgressUi(resetLog) {
    ...
    if (!wrap) {
      ...
      wrap.innerHTML =
        ...
        "</div>" +
        '<div class="ucwc-progress-meta"><strong id="ucwc-progress-pct" class="ucwc-progress-pct">0%</strong></div>' +
      ...
      if (_ucwcLastPct > 0 && _ucwcLastPct < 90) {
        ...
        if (age > 2500 && soft > _ucwcLastPct && soft - _ucwcLastPct <= 8) {
          ...
          if (msg && _ucwcLastStage) {
            ...
            // Status line: stage + elapsed only (percent lives under the bar, left large blue)
            msg.textContent =
              _ucwcLastStage +
              "中…（已用时 " +
              sec +
  ...
  function setProgress(pct, stage, line) {
    ...
    if (msg) {
      ...
      // Status above bar only; large blue % under bar — no duplicate stage beside %
      msg.textContent = String(base).replace(/\s*\d+%\s*/g, " ").replace(/\s{2,}/g, " ").replace(/[…\.]*$/, "") + "…";
    }
  }
  ...
  function alignPathFieldWidths(root) {
    try {
      ...
      // Prefer nearby selects (same block as path fields: mouse / bg / mascot)
      var names = [
        "MOUSE_FX",
        "MOUSE_STYLE",
        "MOUSE_CURSOR",
      ...
      // Dual-upload "从电脑上传" file row: same outer width as path/select
      var files = form.querySelectorAll(
        ".ucwc-upload-src input[type='file'], #ucwc-mouse-cursor-upload, .ucwc-cursor-upload-only input[type='file']"
      );
      for (i = 0; i < files.length; i++) {
        files[i].style.setProperty("width", px, "important");
        files[i].style.setProperty("max-width", "100%", "important");
        files[i].style.setProperty("box-sizing", "border-box", "important");
      }
    } catch (eAlign) {}
  }
  ...
  function wireFileTreePickers() {
    try {
      ...
      if (!$els.length) {
        // legacy ids
        $els = $("#ucwc-bg-local-path, #ucwc-hutao-local-path");
      }
      $els.each(function () {
        var $el = $(this);
        if (!$el.length) return;
        ...
        // Folder-only fields: data-pickfolders must be present for
  ...
  function bindMouseFxControls(root) {
    ...
    // 自定义样式：完全复用 syncUi/hideRow（与壁纸自定义一致）
    (function bindCursorUploadVisibility() {
      var sel =
        root.querySelector("#ucwc-mouse-cursor") ||
        root.querySelector('select[name="MOUSE_CURSOR"]');
      if (!sel) return;
      function bump() {
        try {
          syncUi();
        ...
        try {
          livePreviewMouseFx(root);
        } catch (eLP) {}
      }
      sel.addEventListener("change", bump);
      sel.addEventListener("input", bump);
      bump();
      setTimeout(bump, 0);
      setTimeout(bump, 80);
      setTimeout(bump, 300);
  ...
  function syncScrollbarFormValues(root) {
    root = root || document;
    var globalPairs = [
      ["ucwc-scrollbar-track-global-picker", "ucwc-scrollbar-track-global"],
      ["ucwc-scrollbar-thumb-global-picker", "ucwc-scrollbar-thumb-global"],
      ["ucwc-scrollbar-hover-global-picker", "ucwc-scrollbar-hover-global"]
    ];
    for (var gi = 0; gi < globalPairs.length; gi++) {
      var gp = root.querySelector("#" + globalPairs[gi][0]);
      var gt = root.querySelector("#" + globalPairs[gi][1]);
      if (gp && gt && /^#[0-9a-f]{3,6}$/i.test(String(gp.value || ""))) gt.value = gp.value;
    }
    ["DASHBOARD", "MAIN", "SHARES", "DOCKER", "VMS", "APPS", "USERS", "SETTINGS", "PLUGINS", "TOOLS"].forEach(function (page) {
      var auto = root.querySelector('[data-ucwc-scrollbar-auto="' + page + '"]');
      ["TRACK", "THUMB", "THUMB_HOVER"].forEach(function (part) {
        var picker = root.querySelector("#ucwc-scrollbar-" + part + "-" + page + "-picker");
        var text = root.querySelector("#ucwc-scrollbar-" + part + "-" + page);
        if (auto && auto.checked) {
          if (text) text.value = "";
        } else if (picker && text && /^#[0-9a-f]{3,6}$/i.test(String(picker.value || ""))) {
          text.value = picker.value;
        }
      });
    });
  ...
  function bindScrollbarControls(root) {
    root = root || document;
    var pairs = [
      ["ucwc-scrollbar-track-global-picker", "ucwc-scrollbar-track-global"],
      ["ucwc-scrollbar-thumb-global-picker", "ucwc-scrollbar-thumb-global"],
    ...
    ["DASHBOARD", "MAIN", "SHARES", "DOCKER", "VMS", "APPS", "USERS", "SETTINGS", "PLUGINS", "TOOLS"].forEach(function (page) {
      ["TRACK", "THUMB", "THUMB_HOVER"].forEach(function (part) {
        pairs.push(["ucwc-scrollbar-" + part + "-" + page + "-picker", "ucwc-scrollbar-" + part + "-" + page]);
      });
    });
    pairs.forEach(function (pair) {
      var picker = root.querySelector("#" + pair[0]);
      var text = root.querySelector("#" + pair[1]);
      if (!picker || !text) return;
      picker.addEventListener("input", function () { text.value = picker.value; });
      picker.addEventListener("change", function () { text.value = picker.value; });
      text.addEventListener("input", function () {
        var value = String(text.value || "").trim();
        if (/^#[0-9a-f]{3,6}$/i.test(value)) picker.value = value;
      });
    });
    ["DASHBOARD", "MAIN", "SHARES", "DOCKER", "VMS", "APPS", "USERS", "SETTINGS", "PLUGINS", "TOOLS"].forEach(function (page) {
      var auto = root.querySelector('[data-ucwc-scrollbar-auto="' + page + '"]');
      ...
      var fields = [];
      ["TRACK", "THUMB", "THUMB_HOVER"].forEach(function (part) {
        fields.push(root.querySelector("#ucwc-scrollbar-" + part + "-" + page));
        fields.push(root.querySelector("#ucwc-scrollbar-" + part + "-" + page + "-picker"));
      });
      function sync() {
        var inheriting = !!auto.checked;
        for (var i = 0; i < fields.length; i += 2) {
          if (inheriting && fields[i]) fields[i].value = "";
          if (fields[i]) fields[i].readOnly = inheriting;
          if (fields[i + 1]) fields[i + 1].disabled = inheriting;
        }
      }
      auto.addEventListener("change", sync);
      for (var j = 1; j < fields.length; j += 2) {
        if (fields[j]) fields[j].addEventListener("click", function () { if (auto.checked) { auto.checked = false; sync(); } });
      }
      sync();
    });
  }
  try { bindScrollbarControls(document); } catch (eScrollbarBind) {}
