  ...
  function bindFormUi() {
    try {
      ...
      form.addEventListener("submit", function (ev) {
        ...
        // Any file field with a selected file must use multipart FormData.
        // Old logic only treated wallpaper-custom as "uploading", so custom mascot GIF
        // was posted as urlencoded (file skipped) → "请先选择并上传自定义吉祥物 GIF".
        var hasMascotUpload = !!(
          mascotFile &&
          mascotFile.files &&
          mascotFile.files.length > 0
        );
        var hasBgUpload = !!(file && file.files && file.files.length > 0);
        var uploading = hasUploadFile();
        ...
        fetch("/plugins/custom.css/ucwc-theme-fx-save.php", fetchOpts)
          .then(function (r) {
