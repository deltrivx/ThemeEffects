...
(function (global) {
  ...
  function isUiPlaying() {
    // Strict: play/pause icon must match real audio only.
    // Cross-page resume intent / retry may keep trying, but never fake "playing" UI when paused.
  ...
  function updateChipUi() {
    ...
    if (chipEls.lrc) {
      ...
      else if (audio && !audio.paused) chipEls.lrc.textContent = "♪ 播放中";
  ...
  function mount() {
    ...
    if (!shouldShowCard()) {
      ...
      updateModeBtns();
      // Sync button to live audio after cross-page mount (do not force playing icon)
