...
(function () {
  ...
  var PATCHED = false;
  var suggestRafId = 0;
  var dashboardSelectRafId = 0;
  var dashboardSelectOpen = null;
  ...
  function closeDashboardSelectMenu() {
    if (!dashboardSelectOpen) return;
    var state = dashboardSelectOpen;
    dashboardSelectOpen = null;
    state.button.setAttribute("aria-expanded", "false");
    state.menu.classList.remove("is-open");
  }

  function positionDashboardSelectMenu(state) {
    if (!state || dashboardSelectOpen !== state) return;
    var rect = state.button.getBoundingClientRect();
    var edge = 8;
    var width = Math.max(rect.width, 44);
    width = Math.min(width, window.innerWidth - edge * 2);
    var menuHeight = Math.min(state.menu.scrollHeight || 160, 220);
    var below = window.innerHeight - rect.bottom - edge;
    var top = rect.bottom + 4;
    if (below < Math.min(menuHeight, 160) && rect.top > menuHeight + edge) {
    ...
    state.menu.style.maxHeight = Math.max(96, Math.min(220, window.innerHeight - edge * 2)) + "px";
  }

  function scheduleDashboardSelectPosition() {
    if (!dashboardSelectOpen || dashboardSelectRafId) return;
    dashboardSelectRafId = window.requestAnimationFrame(function () {
      dashboardSelectRafId = 0;
      positionDashboardSelectMenu(dashboardSelectOpen);
    });
  }

  function selectDashboardOption(state, index) {
    if (!state || !state.select || index < 0 || index >= state.select.options.length) return;
    state.select.selectedIndex = index;
    state.button.firstChild.nodeValue = state.select.options[index].textContent || "";
    var options = state.menu.querySelectorAll(".ucwc-dashboard-select-option");
    for (var i = 0; i < options.length; i++) {
      options[i].setAttribute("aria-selected", i === index ? "true" : "false");
    }
    state.select.dispatchEvent(new Event("change", { bubbles: true }));
    closeDashboardSelectMenu();
    state.button.focus();
  }

  function openDashboardSelectMenu(state) {
    if (!state) return;
    if (dashboardSelectOpen && dashboardSelectOpen !== state) closeDashboardSelectMenu();
    dashboardSelectOpen = state;
    state.button.setAttribute("aria-expanded", "true");
    state.menu.classList.add("is-open");
    positionDashboardSelectMenu(state);
  }

  function enhanceDashboardSelect(select) {
    if (!select || select.__ucwcDashboardSelect) return;
    if (!select.closest || !select.closest("table.dashboard")) return;
    var rect = select.getBoundingClientRect();
    var wrapper = document.createElement("span");
    wrapper.className = "ucwc-dashboard-select-wrap";
    wrapper.style.width = Math.max(40, Math.round(rect.width || 80)) + "px";
    var button = document.createElement("button");
    button.type = "button";
    button.className = "ucwc-dashboard-select-button";
    button.setAttribute("aria-haspopup", "listbox");
    button.setAttribute("aria-expanded", "false");
    button.setAttribute("aria-label", select.getAttribute("aria-label") || select.name || "选择");
    var menu = document.createElement("div");
    menu.className = "ucwc-dashboard-select-menu";
    menu.setAttribute("role", "listbox");
    menu.setAttribute("aria-label", button.getAttribute("aria-label"));
    var state = { select: select, wrapper: wrapper, button: button, menu: menu };
    for (var i = 0; i < select.options.length; i++) {
      var option = select.options[i];
      var item = document.createElement("div");
      item.className = "ucwc-dashboard-select-option";
      ...
      (function (index) {
        item.addEventListener("click", function (ev) {
          ev.preventDefault();
          ev.stopPropagation();
          selectDashboardOption(state, index);
        });
      })(i);
      menu.appendChild(item);
    }
    button.appendChild(document.createTextNode(select.options[select.selectedIndex]?.textContent || ""));
    button.addEventListener("click", function (ev) {
      ev.preventDefault();
      ev.stopPropagation();
      if (dashboardSelectOpen === state) closeDashboardSelectMenu();
      else openDashboardSelectMenu(state);
    });
    button.addEventListener("keydown", function (ev) {
      ...
      if (ev.key === "Enter" || ev.key === " ") {
        ev.preventDefault();
        openDashboardSelectMenu(state);
        return;
      }
      if (ev.key === "ArrowDown" || ev.key === "ArrowUp") {
        ev.preventDefault();
        var next = select.selectedIndex + (ev.key === "ArrowDown" ? 1 : -1);
        if (next < 0) next = select.options.length - 1;
        if (next >= select.options.length) next = 0;
        selectDashboardOption(state, next);
      }
    });
    select.__ucwcDashboardSelect = state;
    select.classList.add("ucwc-dashboard-select-native");
    select.parentNode.insertBefore(wrapper, select);
    wrapper.appendChild(button);
    wrapper.appendChild(select);
    document.body.appendChild(menu);
    select.addEventListener("change", function () {
      if (button.firstChild) button.firstChild.nodeValue = select.options[select.selectedIndex]?.textContent || "";
      var options = menu.querySelectorAll(".ucwc-dashboard-select-option");
      for (var j = 0; j < options.length; j++) options[j].setAttribute("aria-selected", j === select.selectedIndex ? "true" : "false");
    });
  }

  function initDashboardSelectMenus() {
    if (!isDashboardPage()) return false;
    var selects = document.querySelectorAll("table.dashboard .tile-header select");
    var found = false;
    for (var i = 0; i < selects.length; i++) {
      enhanceDashboardSelect(selects[i]);
      found = true;
    }
    return found;
  }
  ...
  function bootDashboardSelectMenus() {
    initDashboardSelectMenus();
    document.addEventListener("pointerdown", function (ev) {
      if (!dashboardSelectOpen) return;
      if (ev.target.closest && (ev.target.closest(".ucwc-dashboard-select-menu") || ev.target.closest(".ucwc-dashboard-select-wrap"))) return;
      closeDashboardSelectMenu();
    }, true);
    document.addEventListener("keydown", function (ev) {
      if (ev.key === "Escape") closeDashboardSelectMenu();
    }, true);
    window.addEventListener("resize", scheduleDashboardSelectPosition, { passive: true });
    window.addEventListener("scroll", scheduleDashboardSelectPosition, { passive: true, capture: true });
    var attempts = 0;
    var timer = window.setInterval(function () {
      attempts += 1;
      if (initDashboardSelectMenus() || attempts >= 80) window.clearInterval(timer);
    }, 100);
  }
