  ...
  function showThemeUpdate(data) {
    ...
    dialog.querySelectorAll("[data-mode]").forEach(function (button) {
      button.addEventListener("click", function () {
        ...
        result.textContent = "正在启动更新任务…";
        var params = new URLSearchParams();
        params.set("UCWC_ACTION", "install_version");
        params.set("version", latest);
        params.set("install_mode", button.getAttribute("data-mode"));
        ...
        window.fetch("/plugins/theme.effects/ucwc-update.php", {
          ...
          .then(function (reply) {
            if (!reply.ok) throw new Error(reply.error || reply.message || "无法启动更新任务。");
            result.textContent = reply.message || "更新任务已启动。完成后刷新页面即可查看新版本。";
          })
          .catch(function (error) {
            result.textContent = error.message || "无法启动更新任务。";
