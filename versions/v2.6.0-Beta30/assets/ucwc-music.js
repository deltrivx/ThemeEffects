...
(function (global) {
  ...
  function bindGestureUnlock() {
    ...
    var tryUnlock = function (ev) {
      try {
        ...
        // iOS: resume AudioContext if any (we use HTMLAudio; still touch-sync play)
        var ok = false;
        try {
          ok = tryResumeFromSession(true);
        } catch (eR) {
          ok = false;
        }
        if (!ok && state.tracks.length) {
          try {
            playAt(state.index, true, pendingSeekTo > 0.5 ? pendingSeekTo : undefined);
          } catch (eP) {}
        }
        setTimeout(function () {
          unlocking = false;
    ...
    try {
      ...
      document.addEventListener("pointerup", tryUnlock, opts);
  ...
  function maybeResumeOrAutoplay() {
    ...
    if (sessionWantsResume(sess) || resumeIntent) {
      if (isDashboard() || isSitewidePlay()) {
        ...
        updateMeta();
        renderList();
        // Mobile: arm gesture unlock BEFORE play() so the same navigation tap
        // chain / next tap can unlock (iOS often blocks the first programmatic play).
        gestureResumeWanted = true;
        bindGestureUnlock();
        playAt(idx, true, t);
        if (isSitewidePlay()) {
  ...
  function mount() {
    ...
    // Mobile / autoplay-policy: bind unlock early so first tap after nav works
    bindGestureUnlock();
    if (isSitewidePlay()) showResumeChip();
