...
(function () {
    ...
    try {
      ...
      form.addEventListener("submit", function (ev) {
        ...
        var fieldSets = {
          ...
          perf: ["GPU_ACCEL", "PERF_PROFILE", "CLIENT_OPTIMIZED"],
          all: null,
        };
  ...
  function mountVersionInPageTitle() {
    try {
      ...
      if (existing && (existing.querySelector("#ucwc-btn-check") || existing.querySelector("#ucwc-service-toggle"))) {
        // already mounted
  ...
  function applyPerfSuggestion(tier, opts) {
    ...
    if (tier === "low") {
      ...
      if (particles && particles.value === "yes" && pCount) {
        var n = parseInt(pCount.value, 10) || 60;
        if (n > 80) pCount.value = "80";
      }
  ...
  function quietSavePerf(opts) {
    ...
    if (section === "all") {
      ...
      ["GPU_ACCEL", "PERF_PROFILE", "CLIENT_OPTIMIZED"].forEach(function (nm) {
        var node = form.querySelector('[name="' + nm + '"]');
        if (node) params.set(nm, node.value);
      });
    }
    var headers = {
  ...
  function showPerfToast(probe) {
    try {
      ...
      var boot = window.__UCWC_FX_BOOT__ || {};
      var gpuLine = boot.server_gpu
        ? "服务器已检测到核显/显卡，GPU 加速建议：自动开启（仅浏览器合成）。"
        : "服务器未检测到 GPU，GPU 加速建议：关闭。";
      ...
      el.innerHTML =
        ...
        "。<br>" +
        gpuLine +
        "<br>可一键应用建议配置，避免低配设备卡顿；也可稍后在「性能」段自行调整。</div>" +
      ...
      document.getElementById("ucwc-perf-apply").onclick = function () {
        ...
        quietSavePerf({ section: "all" }).finally(function () {
          el.remove();
          try {
            window.location.reload();
          } catch (e) {}
