...
function ucwc_fx_save($path, $fx) {
    $keys = ["PARTICLES","PARTICLES_COUNT","HUTAO","HUTAO_SIZE","HUTAO_POS","HUTAO_TYPE","HUTAO_BLUR","HUTAO_BLUR_LEVEL","REDUCE_MOTION","BG_MODE","BG_LOCAL_SLOT","BG_GALLERY","BG_CUSTOM_API","BG_BLUR","BG_BLUR_LEVEL","FONT_BODY","FONT_TITLE","FONT_TITLE_UPPER","FONT_SIZE","FONT_TITLE_SIZE","FONT_CUSTOM_BODY","FONT_CUSTOM_TITLE","FONT_LOCAL_BODY","FONT_LOCAL_TITLE","COLOR_PRESET","COLOR_TEXT","COLOR_TITLE","COLOR_LABEL","GPU_ACCEL","PERF_PROFILE","CLIENT_OPTIMIZED"];
    $lines = [];
    foreach ($keys as $k) {
...
if ($doPerf) {
    $ga = strtolower(trim((string)($_POST["GPU_ACCEL"] ?? ($fx["GPU_ACCEL"] ?? "auto"))));
    $fx["GPU_ACCEL"] = in_array($ga, ["auto", "on", "off"], true) ? $ga : "auto";
