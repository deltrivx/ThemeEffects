...
(function () {
  ...
  function setProgress(pct, stage, line) {
    ...
    if (stage) {
      ...
      // Stage text only — percent is solely the large left blue under the bar
      if (stageEl) stageEl.textContent = stage;
    } else if (stageEl) {
      stageEl.textContent = _ucwcLastStage || "进行中";
    }
    if (msg) {
      var base = line || stage || _ucwcLastStage || "执行中…";
      // Do not inject extra % into the status line above the bar
