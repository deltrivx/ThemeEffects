...
(function () {
    ...
    try {
      ...
      form.addEventListener("submit", function (ev) {
        ...
        var fieldSets = {
          ...
          mouse: ["MOUSE_FX", "MOUSE_STYLE", "MOUSE_SIZE", "MOUSE_SIZE_COMMIT", "MOUSE_INTENSITY", "MOUSE_INTENSITY_COMMIT", "MOUSE_COLOR", "MOUSE_COLOR_MODE", "MOUSE_CLICK_RIPPLE"],
  ...
  /**
   * Newest beta that is still newer than the current stable latest (and local if present).
   * If every beta is older/equal to the formal release, return null →「无最新 Beta 版」.
   */
  function pickBetaLatest(data, opts) {
    ...
    // Floor = max(stable, local-if-stable-or-any) so beta must be a real forward candidate
    if (localId && !isBetaVersionId(localId) && isVersionNewer(localId, floorId || "0")) {
      floorId = localId;
    } else if (localId && isBetaVersionId(localId) && isVersionNewer(localId, floorId || "0")) {
      // local is already on a beta ahead of stable — only offer newer betas
      floorId = localId;
    }
    var best = null;
    var i, v;
  ...
  function showCheck(data, opts) {
    ...
    if (betaMode) {
      // Only "new beta" when pickBetaLatest returned a forward candidate
      updateAvail = !!(latestMeta && latestMeta.id);
      ...
      if (!updateAvail && local.installed && localVerRaw && latestMeta.id !== localVerRaw) {
        // equal string miss but same core, or local ahead — not an upgrade
    ...
    if (betaMode && (!latestMeta || !latestMeta.id)) {
      ...
      if (updateAvail) {
        ...
        html +=
          '<p class="ucwc-ok">' +
          (betaMode
            ? "已是当前可用的最新 Beta（或同版本）。仍可重新安装该 Beta 包。"
  ...
  function wireFileTreePickers() {
    try {
      ...
      var $ = jQuery;
      var ids = ["#ucwc-bg-local-path", "#ucwc-hutao-local-path"];
      for (var i = 0; i < ids.length; i++) {
        var $el = $(ids[i]);
        if (!$el.length) continue;
        if ($el.data("ucwcFileTree")) continue;
        ...
        if (!$el.attr("data-pickcloseonfile")) $el.attr("data-pickcloseonfile", "true");
        $el.fileTreeAttach();
