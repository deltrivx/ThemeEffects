 ...
 * Cross-page: in-page floating chip (embedded overlay, never window.open/new tab).
 * Sitewide: chip always visible on Unraid pages; card ↔ chip bi-directional sync.
 * Clicks only control playback — no navigation. Full reload uses session resume.
 */
(function (global) {
  "use strict";
  ...
  function isUiPlaying() {
    // Strict: only real in-page audio. Never open popup/new tab.
  ...
  function savePlaySession(forcePlaying) {
    try {
      ...
      if (audio && isFinite(audio.currentTime) && audio.currentTime > 0.25) {
        // live wins unless this is a failed seek-to-0 flash while we still have a target
   ...
   * - Keeps previous <img> visible until the new src loads (no blank flash).
   * - onerror must NOT tear down a still-valid previous cover for another request.
   * - optional onReady(ok) fires after load/error settles for this assignment.
   */
  function setCoverUrl(url, trackId, onReady) {
    ...
    if (!url) {
      // Never blank a good cover just because a caller passed empty —
      // explicit wipes go through clearCoverDom / clearCoverView.
      if (typeof onReady === "function") onReady(false);
      return;
    }
    var finalUrl = absPluginUrl(url);
    var img = els.art.querySelector("img.ucwc-music-art-img");
    var hadPrev = !!(img && img.getAttribute("src"));
    if (!img) {
    ...
    var done = false;
    // Dedicated paint generation so loadCover seq and img assign don't collide.
    ...
    function markOk() {
      ...
      // Accept paint if this track is still current OR still the one we intended while loading next
      if (state.cover.id !== trackId && state.cover.paintedId !== trackId) {
        // If user moved on, still keep pixels if nothing newer painted
        if (state.cover.paintedId && state.cover.paintedId !== trackId) {
          if (typeof onReady === "function") onReady(false);
          return;
        }
      }
      if (state.cover.id === trackId || !state.cover.paintedId) {
    ...
    img.onerror = function () {
      ...
      if (state.cover.paintGen !== assignGen) return;
      // Only drop art if THIS broken src is still attached and we have nothing better.
      var curSrc = "";
      try {
      ...
      // If a previous cover for another paint is still desired, keep shell
      if (hadPrev && state.cover.paintedId && state.cover.paintedId !== trackId) {
        if (typeof onReady === "function") onReady(false);
        return;
      }
      // Same-track failure with no replacement: fall back to placeholder
    ...
    // Swap src without removing node — previous pixels stay until decode of new src
    try {
      img.style.opacity = "1";
      img.style.visibility = "visible";
    } catch (e4) {}
  ...
  function buildUi() {
    ...
    els.play.addEventListener("click", btnOnly(function () { togglePlay(); }));
    els.prev.addEventListener("click", btnOnly(function () { prev(); }));
    ...
    // Card shell: bubble-phase only so control handlers still receive the click.
    // Never capture-stop on the card root (that blocked play/prev/next entirely).
    ...
    if (sessionWantsResume(sess) || resumeIntent) {
      if (isDashboard() || isSitewidePlay()) {
        ...
        if (isSitewidePlay()) {
          gestureResumeWanted = true;
          bindGestureUnlock();
          showResumeChip(audio && !audio.paused ? "" : "续播中…");
  ...
  function mount() {
    ...
    if (!shouldShowCard()) {
      ...
      updateModeBtns();
      // Sync button to live audio after cross-page mount
