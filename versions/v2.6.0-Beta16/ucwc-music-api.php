...
/**
 * Resolve cover: local → cache → network download.
 * @return array{0:string,1:string,2:string} [absPath, source, err]
 */
function m_resolve_cover($audioAbs, $rel = "", $doFetch = true) {
    ...
    if ($cacheDir !== "") {
        foreach (["jpg", "png", "webp"] as $ext) {
            $cp = $cacheDir . "/" . $ckey . "." . $ext;
            if (is_file($cp) && @filesize($cp) > 64) return [str_replace("\\", "/", $cp), "cache", ""];
        }
    }
    if (!$doFetch || $title === "") return ["", "none", $title === "" ? "no title" : "no cover"];

    $url = m_fetch_itunes_cover_url($artist, $title, $album);
    $srcTag = "itunes";
    if ($url === "") {
        $url = m_fetch_netease_cover_url($artist, $title);
        $srcTag = "netease";
    }
    ...
    if ($data === false || !m_is_cover_bytes($data)) {
        ...
        if ($srcTag === "itunes") {
            $url2 = m_fetch_netease_cover_url($artist, $title);
            if ($url2 !== "" && $url2 !== $url) {
        ...
        if ($data === false || !m_is_cover_bytes($data)) {
            mlog("cover-download fail2 src=$srcTag err=$err");
            return ["", "none", $err !== "" ? $err : "封面下载失败"];
        }
    }
    ...
    if ($written === "") return ["", "none", "无法写入封面缓存"];
    mlog("cover-ok src=$srcTag title=$title dst=$written bytes=" . strlen($data));
...
if ($action === "cover") {
    ...
    if (isset($_GET["raw"]) && !in_array(strtolower((string)$_GET["raw"]), ["0", "no", "false", "off"], true)) {
        [$cpath, $src, $cerr] = m_resolve_cover($abs, $rel, $doFetch);
        ...
        if (!$ok && $cacheDir !== "") {
            ...
            if (!$ok && strpos(str_replace("\\", "/", $cpath), $cd2) === 0) {
                $ok = true;
                $cReal = str_replace("\\", "/", $cpath);
            }
        }
