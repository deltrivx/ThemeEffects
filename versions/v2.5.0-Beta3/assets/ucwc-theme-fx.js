...
(function () {
  ...
  function showCheck(data, opts) {
    ...
    body.innerHTML = html;
    actions.innerHTML = "";
    // Beta：无 forward 时不提供安装旧 Beta；有 forward 时安装/重装该包
    if (!betaMode || (latestMeta && latestMeta.id)) {
      var btn = document.createElement("input");
      btn.type = "button";
      if (betaMode) {
        btn.value =
          betaState === "update" || betaState === "install"
            ? "安装此 Beta"
            : "重新安装此 Beta";
        btn.addEventListener("click", function () {
          runInstall("install_version", (installTarget && installTarget.id) || latestMeta.id);
        });
      } else {
