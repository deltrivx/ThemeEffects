...
(function (global) {
  ...
  function isUiPlaying() {
    // Truthful UI: prefer real audio state. Soft "playing" only while retry loop is active
    // AND element already has a source (resume in progress), not on bare page load flash.
    ...
    if (
      ...
      pendingResume &&
      playRetryCount > 0 &&
      (resumeIntent || gestureResumeWanted) &&
      Date.now() < uiPlayStableUntil
    ) {
      return true;
    }
    return false;
