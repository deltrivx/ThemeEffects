...
(function () {
  ...
  function reassertCursor() {
    ...
    applyCursorStyle();
    // Force paint: some skins re-apply inline cursor after class change
    try {
      var cur = normalizeCursor(api.cursor);
      if (cur !== "system") {
        var root = document.documentElement;
        var body = document.body;
        var cs = root ? window.getComputedStyle(root).getPropertyValue("--ucwc-cursor") : "";
        var val = (cs && cs.trim()) || root.style.getPropertyValue("--ucwc-cursor") || "";
        if (val) {
          if (root && !root.style.cursor) {
            /* leave class-driven; only clear stale inline system cursors on body */
          }
          if (body && body.style && body.style.cursor && /pointer|default|auto|move|grab|col-resize|row-resize|ns-resize|ew-resize/i.test(body.style.cursor)) {
            try { body.style.removeProperty("cursor"); } catch (eClr) {}
          }
        }
      }
    } catch (ePaint) {}
  ...
  setTimeout(reassertCursor, 120);
  setTimeout(reassertCursor, 400);
  setTimeout(reassertCursor, 1000);
