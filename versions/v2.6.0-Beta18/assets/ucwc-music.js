...
(function (global) {
  ...
  function savePlaySession(forcePlaying) {
    try {
      ...
      var curT = 0;
      if (audio && isFinite(audio.currentTime) && audio.currentTime > 0) curT = audio.currentTime;
      else {
        // keep previous session time if audio not ready yet
        try {
          var prev = loadPlaySession();
          if (prev && typeof prev.t === "number" && prev.t > 0 && prev.id && t && prev.id === t.id) curT = prev.t;
        } catch (ePrev) {}
      }
      var payload = {
        playing: !!playing,
  ...
  function maybeResumeOrAutoplay() {
    ...
    if (sessionWantsResume(sess) || resumeIntent) {
      if (isDashboard() || isSitewidePlay()) {
        ...
        // Restore track + seek first; then attempt play (may be blocked → no icon flash without src)
        state.index = idx;
        state.playing = false;
        updatePlayBtn();
        playAt(idx, true, t);
        ...
        // Retries only after src applied
        setTimeout(function () {
    ...
    // Not actively playing before refresh: still restore last track + progress (paused)
