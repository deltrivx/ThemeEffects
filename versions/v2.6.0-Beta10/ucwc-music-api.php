...
function m_lrc_cache_key($artist, $title) {
    return substr(sha1(mb_strtolower(trim($artist) . "|" . trim($title), "UTF-8")), 0, 24);
}
...
function m_fetch_and_save_lrc($audioAbs, $rel = "") {
    ...
    if ($body === false) {
        return [false, "", "歌词网络请求失败: " . ($err !== "" ? $err : "empty"), ""];
    }
    ...
    $aNorm = mb_strtolower($artist, "UTF-8");
    foreach ($json as $row) {
        if (!is_array($row)) continue;
        $synced = trim((string)($row["syncedLyrics"] ?? ""));
        if ($synced === "") continue;
        $score = 10;
        $rt = mb_strtolower(trim((string)($row["trackName"] ?? "")), "UTF-8");
        $ra = mb_strtolower(trim((string)($row["artistName"] ?? "")), "UTF-8");
        if ($rt !== "" && ($rt === $tNorm || mb_strpos($rt, $tNorm) !== false || mb_strpos($tNorm, $rt) !== false)) $score += 50;
        if ($aNorm !== "" && $ra !== "" && ($ra === $aNorm || mb_strpos($ra, $aNorm) !== false || mb_strpos($aNorm, $ra) !== false)) $score += 30;
        if (!empty($row["instrumental"])) $score -= 20;
        if ($score > $bestScore) { $bestScore = $score; $best = $row; }
    }
    if ($best === null) return [false, "", "仅有纯文本歌词（无时间轴）", ""];
    ...
    if ($written !== "") {
        mlog("lrc-download ok title={$title} artist={$artist} dst={$written} bytes=" . strlen($text));
