...
(function () {
  ...
  function syncUi() {
    try {
      ...
      // Mouse custom cursor file + hotspot (same hideRow as wallpaper custom)
      ...
      var mouseHotX = root.querySelector('input[name="MOUSE_CURSOR_HOTSPOT_X"]');
      var mouseCursorIsUpload = !!(mouseCursor && mouseCursor.value === "upload");
  ...
  function bindMouseFxControls(root) {
    ...
    // 自定义样式文件 / 热点：与壁纸相同，走 hideRow 整行显隐
    (function bindCursorUploadVisibility() {
      ...
      function syncCursorUploadRows() {
        var show = String(sel.value || "") === "upload";
        var fileEl =
          root.querySelector("#ucwc-mouse-cursor-upload") ||
          root.querySelector('input[name="MOUSE_CURSOR_UPLOAD"]');
        var hx = root.querySelector('input[name="MOUSE_CURSOR_HOTSPOT_X"]');
        var hy = root.querySelector('input[name="MOUSE_CURSOR_HOTSPOT_Y"]');
        // hideRow works like BG_UPLOAD / BG_LOCAL_PATH (whole markdown DL row)
        hideRow(fileEl, !show);
        hideRow(hx, !show);
        // hy is usually same row as hx — hideRow on hx covers the DL; still mark hy inputs usable
        try {
          if (fileEl) {
            fileEl.disabled = !show;
            if (show) fileEl.removeAttribute("disabled");
          }
          if (hx) {
            hx.disabled = !show;
            if (show) hx.removeAttribute("disabled");
          }
          if (hy) {
            hy.disabled = !show;
            if (show) hy.removeAttribute("disabled");
          }
        } catch (eDis) {}
        // also clear any leftover inline display on span wrappers so hideRow can unhide cleanly
        try {
          var wraps = root.querySelectorAll(".ucwc-cursor-upload-only, [data-ucwc-cursor-upload-row]");
          for (var i = 0; i < wraps.length; i++) {
            if (show) {
              wraps[i].style.removeProperty("display");
              if (!wraps[i].style.display) wraps[i].style.display = "inline-flex";
            }
          }
        } catch (eW) {}
      }
      ...
      sel.addEventListener("change", function () {
        syncCursorUploadRows();
        try {
          livePreviewMouseFx(root);
        } catch (eLP) {}
      });
      sel.addEventListener("input", syncCursorUploadRows);
      syncCursorUploadRows();
      setTimeout(syncCursorUploadRows, 0);
      setTimeout(syncCursorUploadRows, 80);
      setTimeout(syncCursorUploadRows, 300);
      setTimeout(syncCursorUploadRows, 800);
    })();
