...
(function (global) {
  ...
  function schedulePlayRetries(reason) {
    ...
    function tick() {
      ...
      if (playRetryCount >= delays.length) {
        ...
        bindGestureUnlock();
        if (!isDashboard() && isSitewidePlay()) syncSitewideChip("点击任意处续播");
        return;
      }
  ...
  function setChipXY(x, y) {
    if (!chip) return;
    var maxX = Math.max(8, (window.innerWidth || 800) - (chip.offsetWidth || 200) - 8);
    var maxY = Math.max(8, (window.innerHeight || 600) - (chip.offsetHeight || 56) - 8);
    x = Math.min(maxX, Math.max(8, x));
    y = Math.min(maxY, Math.max(8, y));
    // CSS uses right/bottom !important — must override with setProperty important
    try {
  ...
  function bindChipDrag() {
    ...
    var onMove = function (ev) {
      ...
      if (Math.abs(dx) + Math.abs(dy) > 3) chipDrag.moved = true;
    ...
    function startDrag(ev) {
      ...
      var t = ev.target;
      if (t && t.closest && t.closest(".ucwc-music-chip-btn")) return;
      chipDrag.on = true;
      chipDrag.moved = false;
      var pt = ev.touches && ev.touches[0] ? ev.touches[0] : ev;
    ...
    chip.addEventListener("touchstart", startDrag, { capture: true, passive: true });
  }
  ...
  function updateChipUi() {
    ...
    if (chipEls.lrc) {
      ...
      chipEls.lrc.textContent = line || (state.playing || (audio && !audio.paused) ? "♪ 播放中" : "点击播放续播");
    }
    if (chipEls.play) {
      chipEls.play.innerHTML = audio && !audio.paused ? svgIcon("pause") : svgIcon("play");
      chipEls.play.title = audio && !audio.paused ? "暂停" : "播放";
        ...
        if (!(a && !a.paused && lastSrcId === t.id && Math.abs((a.currentTime || 0) - seekTo) < 1.25 && seekTo > 0)) {
          ...
          if (p && typeof p.catch === "function") {
            p.catch(function () {
              ...
              if (isDashboard()) {
                setStatus("正在尝试自动续播…（若仍静音请点一下页面）");
              } else {
                setStatus("");
                syncSitewideChip("自动续播中…");
              }
              updateChipUi();
            });
          }
  ...
  function maybeResumeOrAutoplay() {
    ...
    // Further starts only via user gesture (chip / toggle / gesture unlock).
    if (resumeAttempted) {
      if (!isDashboard() && isSitewidePlay()) {
        syncSitewideChip(audio && audio.src ? "点击播放以续播" : "");
        updateChipUi();
      }
      return;
    }
    ...
    if (sessionWantsResume(sess) || resumeIntent) {
      if (isDashboard() || isSitewidePlay()) {
        ...
        if (!isDashboard() && isSitewidePlay()) {
          gestureResumeWanted = true;
          bindGestureUnlock();
          syncSitewideChip(audio && !audio.paused ? "" : "自动续播中…");
        }
  ...
  function mount() {
    ...
    if (!isDashboard() && isSitewidePlay()) syncSitewideChip();
