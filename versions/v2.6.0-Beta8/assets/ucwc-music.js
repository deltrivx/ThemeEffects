...
(function (global) {
  ...
  function showResumeChip(label) {
    ...
    if (!chip) {
      ...
      chip.addEventListener("click", function (ev) {
        ...
        resumeIntent = true;
        var ok = tryResumeFromSession(true);
  ...
  function maybeResumeOrAutoplay() {
    ...
    if (sessionWantsResume(sess) || resumeIntent) {
      if (isDashboard() || isSitewidePlay()) {
        ...
        playAt(idx, true, t);
        // Autoplay often blocked off-dashboard: surface chip + gesture unlock
        if (!isDashboard() && isSitewidePlay()) {
          setTimeout(function () {
            if (!audio || audio.paused) {
              gestureResumeWanted = true;
  ...
  function mount() {
    ...
    // Restore in-memory intent from durable session (full page reload loses JS state)
    var earlySess = loadPlaySession();
    if (sessionWantsResume(earlySess) && isSitewidePlay()) {
      resumeIntent = true;
      gestureResumeWanted = true;
    }
    ...
    if (!state.loaded) {
      ...
      if (sessionWantsResume(sess) && (!audio || audio.paused)) {
        maybeResumeOrAutoplay();
      } else if (
        isSitewidePlay() &&
        !isDashboard() &&
  ...
  function boot() {
    ...
    bootDone = true;
    // 跨页：整页跳转前强制写入 playing 意图（audio 可能已在卸载中 pause）
    try {
      bindNavFlush();
