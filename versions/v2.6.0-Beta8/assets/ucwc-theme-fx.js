...
(function () {
  ...
  function bindMouseFxControls(root) {
    ...
    // 自定义指针文件 / 热点：仅在 MOUSE_CURSOR=upload 时显示（含 dt 标签行）
    (function bindCursorUploadVisibility() {
      ...
      function syncCursorUploadRows() {
        ...
        var marked = root.querySelectorAll(
          '[data-ucwc-cursor-upload-row], .ucwc-cursor-upload-only'
        );
        for (var i = 0; i < marked.length; i++) {
          marked[i].style.display = show ? "inline-flex" : "none";
        }
        try {
          var dts = root.querySelectorAll("dt");
          for (var d = 0; d < dts.length; d++) {
            var txt = (dts[d].textContent || "").replace(/\s+/g, "");
            if (
      ...
      syncCursorUploadRows();
      // Unraid may finish dt/dd layout slightly later
      setTimeout(syncCursorUploadRows, 0);
      setTimeout(syncCursorUploadRows, 120);
    })();
