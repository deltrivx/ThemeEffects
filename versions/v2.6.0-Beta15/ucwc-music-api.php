...
function m_resolve_cover($audioAbs, $rel = "", $doFetch = true) {
    ...
    [$data, $err, $code] = m_http_get($url, 15);
    if ($data === false || !m_is_cover_bytes($data)) {
        mlog("cover-download fail src=$srcTag err=$err");
    ...
    $stem = pathinfo($audioAbs, PATHINFO_FILENAME);
    $side = $dir . "/" . $stem . "." . $ext;
    if (is_dir($dir) && is_writable($dir) && !is_file($side)) {
...
if ($action === "cover") {
    ...
    if (isset($_GET["raw"]) && !in_array(strtolower((string)$_GET["raw"]), ["0", "no", "false", "off"], true)) {
        ...
        // allow cache under plugin dir or under music root
        $ok = false;
    ...
    $url = "/plugins/theme.effects/ucwc-music-api.php?action=cover&raw=1&id=" . rawurlencode($rel) . "&_v=" . (@filemtime($cpath) ?: time());
    mjson([
        "ok" => true,
        "id" => $rel,
        "url" => $url,
