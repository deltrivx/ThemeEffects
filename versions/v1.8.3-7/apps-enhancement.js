/*! apps-enhancement.js
 * unraid-custom-webui-css v1.8.3-7
...
(function () {
  ...
  /* Dashboard / Docker: missing icons (404 or non-web paths like /mnt/user/...)
   * use Unraid's bare onerror=this.src='…question.png'. On some mobile browsers
   * a failed fallback (or 302 HTML) re-fires onerror forever → continuous flicker.
   * Capture once, lock to question.png or a 1×1 data URI, never loop. */
  ...
  function lockIconFallback(img, useData) {
    try {
      ...
      var next = useData ? UCWC_Q_DATA : UCWC_Q_ICON;
  ...
  function stabilizeIcon(img) {
    if (!isDashDockerIcon(img)) return;
    if (img.dataset && img.dataset.ucwcIconLocked === "1") return;
    try {
      // Already the stock question mark — still pin size class, no loop risk if ok
      var src = img.getAttribute("src") || "";
      if (src.indexOf("question.png") >= 0) {
        if (img.classList) img.classList.add("ucwc-icon-fallback");
      }
      // Non-http paths under /mnt never load in browser
      if (
        src.indexOf("/mnt/") === 0 ||
        src.indexOf("file:") === 0 ||
        (src.indexOf("http") !== 0 &&
          src.indexOf("/") === 0 &&
          src.indexOf("/plugins/") !== 0 &&
          src.indexOf("/state/") !== 0 &&
          src.indexOf("/webGui/") !== 0)
      ) {
        lockIconFallback(img, false);
        return;
      }
      // Replace bare onerror so Unraid's handler cannot loop
      img.onerror = function () {
        var alreadyQ =
          (img.getAttribute("src") || "").indexOf("question.png") >= 0 ||
          (img.dataset && img.dataset.ucwcTriedQ === "1");
        if (img.dataset) img.dataset.ucwcTriedQ = "1";
        lockIconFallback(img, !!alreadyQ);
      };
      // Decode-failed / 0×0 after "load"
      if (img.complete && img.naturalWidth === 0 && src && src.indexOf("data:") !== 0) {
        var alreadyQ2 =
          src.indexOf("question.png") >= 0 ||
          (img.dataset && img.dataset.ucwcTriedQ === "1");
        if (img.dataset) img.dataset.ucwcTriedQ = "1";
        lockIconFallback(img, !!alreadyQ2);
      }
    } catch (e2) {}
  }
