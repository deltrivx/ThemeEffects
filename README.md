...
> 当前正式版：**v2.8.3** · 插件 ID：`theme.effects` · 最低 Unraid：**6.12.0**
...
```bash
curl -fsSL https://github.com/deltrivx/ThemeEffects/releases/download/v2.8.3/install.sh -o /tmp/theme-effects-install.sh
sh /tmp/theme-effects-install.sh install v2.8.3 ota
```
...
```bash
curl -fsSL https://github.com/deltrivx/ThemeEffects/releases/download/v2.8.3/install.sh -o /tmp/theme-effects-install.sh
sh /tmp/theme-effects-install.sh install v2.8.3 full
```
