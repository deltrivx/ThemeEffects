...
最新 Beta：**v2.6.0-Beta11**（transform拖动chip、静音解锁续播、网易云歌词、鼠标壁纸式hideRow）
...
- 吉祥物（内置胡桃 / 自定义 GIF）
- **音乐组件 V1（Beta）**：仪表盘双栏音乐卡片（曲目⇄歌词、**本地 LRC 同步**）+ 可选全站续播（localStorage + 手势/chip）+ 本地目录音源（默认关闭）
- 应用页增强（侧栏、搜索建议、路由隔离）
- 设置页「主题特效」分段「应用」
...
- 音乐 / 指针 Beta：`bash scripts/install.sh install v2.6.0-Beta11 ota`
